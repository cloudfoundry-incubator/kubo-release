...
describe 'kube-apiserver' do
  ...
  it 'explicitly enables only the admission controllers that are marked as true' do
    rendered_kube_apiserver_bpm_yml = compiled_template(
      'kube-apiserver',
      'config/bpm.yml',
      {
    ...
    bpm_yml = YAML.safe_load(rendered_kube_apiserver_bpm_yml)
    expect(bpm_yml['processes'][0]['args']).to include('--enable-admission-plugins=DefaultStorageClass,LimitRanger')
  end

  it 'explicitly disables the admission controllers that are marked as false' do
    rendered_kube_apiserver_bpm_yml = compiled_template(
      'kube-apiserver',
      'config/bpm.yml',
      {
    ...
    bpm_yml = YAML.safe_load(rendered_kube_apiserver_bpm_yml)
    expect(bpm_yml['processes'][0]['args']).to include('--disable-admission-plugins=DenyEscalatingExec,LimitRanger')
  end
